...
export async function runAgent(userInput: string) {
    ...
    const historyMessages = recentContext.map((msg: any) => ({
        role: msg.role,
        content: msg.content
    }));
    ...
    for (let i = 0; i < 5; i++) {
        ...
        for (const toolCall of message.tool_calls) {
            ...
            if (functionName === "get_current_time") {
                ...
                functionResponse = await evolution_tool.handler(functionArgs as any);
                ...
                functionResponse = await updater_tool.handler(functionArgs as any);
            }
